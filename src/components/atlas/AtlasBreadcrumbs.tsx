...
/** Mono breadcrumb trail with hairline separators. The final crumb is marked as the current page. */
      ...
      <ol className="flex flex-wrap items-center gap-x-2 gap-y-1 font-mono text-[11px] font-medium uppercase tracking-[0.16em] text-slate-500">
        {items.map((item, index) => (
          <li key={`${item.label}-${index}`} className="flex min-w-0 items-center gap-2">
            {index > 0 ? <span aria-hidden="true" className="h-px w-3 shrink-0 bg-[#CBD5E1]" /> : null}
            {item.href ? (
